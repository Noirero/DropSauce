...
class KotatsuApp : BaseApp() {
	...
	override fun onCreate() {
		...
		// ACRA handles Java/Kotlin exceptions. Android 11+ additionally records ANRs and native
		// crashes after the process is gone, so recover those records on the next normal launch.
		runCatching { CrashLogStore.capturePreviousSystemExit(this) }
	...
	private inner class RecoveredCrashDialogCallbacks : Application.ActivityLifecycleCallbacks {
		override fun onActivityResumed(activity: Activity) {
			...
			val log = CrashLogStore.pendingLog(activity) ?: return
